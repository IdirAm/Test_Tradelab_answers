var cities = [
  ...
  },
  {
	...
	function showWeather(data) {
		console.log(data);
		$(".actual-icon").empty();
		var actualIconClass= 'wi wi-icon-'+data.weather[0].id;
		$(".actual-icon").append("<div class='"+actualIconClass+"'></div>");
		$(".actual-temp").empty().append(data.main.temp);
		$(".actual-weather").empty().append(data.weather[0].main);
	}
	
	var select = document.getElementById('select-cities');
	
	var uri = 'http://api.openweathermap.org/data/2.5/weather?q=';
	var appid = 'appid=f8e5edda493b5d2f712961cdb9ba617b';

	function getActualWeather(city) {	
		toggleLoader();
		$.ajax({
			url: uri + selectedCity + '&' + appid,
			type: 'GET',
			cache: false,
			error: function(){
				alert("Une erreur innatendue est survenue.");
				toggleLoader();
			},
		...
		getActualWeather(selectedCity);

		// Get current city (onChange)
		select.addEventListener('change', function() {
			selectedCity = this.options[select.selectedIndex].text;
			getActualWeather(selectedCity);
		});	